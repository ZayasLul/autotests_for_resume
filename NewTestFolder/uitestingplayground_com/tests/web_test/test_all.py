...
from NewTestFolder.uitestingplayground_com.tests.web_test.web_base import WebBase
...
class TestAll(WebBase):
    ...
    def test_class_attribute(self):
        ...
        message = self.APP.web_base.get_text_alert()
        # Нажимаем "Ок"
        self.APP.web_base.click_accept_in_alert()
    ...
    def test_client_side_delay(self):
        ...
        # Нажимаем на кнопку
        self.APP.framework_client_side_delay.click__button_client_side_delay()
        # Ожидаем загрузки
    ...
    def test_shadow_dom(self): # Данный тест будет всегда выдавать ошибку по 2 причинам (1- не работает кнопка копирования, совсем, 2 - при получении из значения из поля оно выделяется но не копируется, причина не известна)
        # Открываем страницу
